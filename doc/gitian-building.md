...
*Setup instructions for a Gitian build of FBTC Core using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the FBTC
...
- [Building FBTC Core](#building-FBTC-core)
...
in the FBTC repository under 'Fetch and create inputs' to install sources which require
...
To build FBTC Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the FBTC repository.
...
Output from `gbuild` will look something like
    ...
    From https://github.com/FBTC-project/FBTC
...
For example:
```bash
URL=https://github.com/thrasher-/FBTC.git
...
and the FBTC git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../FBTC/contrib/gitian-descriptors/*|sort|uniq )
...
SIGPATH=/some/root/path/FBTC-detached-sigs
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/FBTC-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/FBTC-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/FBTC-osx-build.assert
```
...
[FBTC-project/gitian.sigs.FBTC](https://github.com/FBTC-project/gitian.sigs.FBTC/) repository, or if that's not possible create a pull
